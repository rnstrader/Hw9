...
Now we have created a tuned model of 4 different types, we will now select using rmse to extract the best parameters for each model
```{r}
best_lasso <- select_best(lasso_tune, "rmse")
best_tree <- select_best(tree_tune, "rmse")
best_bag <- select_best(bag_tune, "rmse")
best_rf <- select_best(rf_tune, "rmse")
```
